# Agora Village
### Deployed and feature complete. 
IT HAS A TWIN PROJECT [Agora Village Evals](https://github.com/hiuyear/agora-evals) which measures what the agents actually do.
...
Every LLM call is instrumented with OpenTelemetry, using the `gen_ai.*` semantic convention (model, token counts, latency) so the traces are readable by any OTel-aware backend, not just one vendor. They're exported to Braintrust and mirrored to a local JSONL file. The behavioral metrics below are computed from Postgres, though — not from the trace store, so a Braintrust outage can't block the report.
...
Next.js (App Router), TypeScript, Tailwind. Supabase for Postgres and Realtime. Anthropic and OpenAI APIs. Zod for validating anything that crosses a runtime boundary. Vercel Workflow DevKit for durable execution. OpenTelemetry for LLM call tracing, exported to Braintrust. Deployed on Vercel.
...
- Supabase credentials (both the service-role and anon keys)
- API keys for both LLM providers
- *(Optional)* `BRAINTRUST_API_KEY` and `BRAINTRUST_PROJECT_NAME` — without these, traces still get written to `traces/spans.jsonl` locally

For live updates to broadcast, `turns` and `decisions` have to be added to the `supabase_realtime` publication in the Supabase dashboard.
